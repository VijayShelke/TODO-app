...
router.get('/insertUpdateTODO', function(req, res, next) {
  ...
  if(new_entry!='') {
    global.mongo.collection(user_name, function (err, collection) {
      ...
      collection.find({"note": new_entry},{note:1},function(err,item){
          if(item.length!=0)
          {
            console.log('entry already present rename it');
            collection.update({"note": new_entry},{done:0 , "date": "12/10/94"}, function (err, items) {
              res.end('');
            });

          }else{
            collection.insert({"note": new_entry,done:1 ,"date": "12/10/94"},{upsert:false}, function (err, items) {
              res.end('');
            });
          }
      });
    ...
    collection.find({user_name:uname}).toArray(function(err, items) {     //check once again username
      ...
      if (items.length === 0 )
      {
        ...
        global.mongo.createCollection(collName, function (err, collection1) {
          collection1.insert({"note": "hello" , "date" : "11/10/12"});
